-- Porra Mundial 2026 — la tabla solo cuenta lo que estaba ABIERTO al unirse al grupo
-- Regla (instructivo): si pagas/te unes tarde, pierdes los puntos de los eventos ya
-- cerrados al entrar, aunque los hubieras pronosticado en tu cuenta (pool_id null).
--   · Partidos (y marcadores exactos): cuentan solo si kickoff > joined_at.
--   · Especiales: cuentan solo si te uniste antes del deadline de especiales.
--   · Cuadro: cuenta solo si te uniste antes del cierre (1er 16avo).
-- Quien paga a tiempo se une antes de todos esos cierres → no se le filtra nada.
...
begin
  ...
  -- Marcadores exactos (mismo gate de fecha de ingreso).
  exact_pts as (
